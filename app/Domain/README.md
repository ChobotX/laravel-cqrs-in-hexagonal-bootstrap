...
- **Cross-domain imports limited to dispatchable/data Contract subdirs** — `App\Domain\{ContextA}` may reference `App\Domain\{ContextB}` only through `Contract\{Command|Query|Event|Entity|ValueObject|Enum|Exception}`. Cross-module `Repository` and `Service` imports are forbidden — use the bus (or a promoted `App\Contract\*` port) instead (`DomainCrossDomainImportsRule`, `DomainCrossDomainGroupUseImportsRule`)
...
- **Infrastructure cross-domain imports** — For Infrastructure classes outside the excluded wiring namespaces (bus, provider, persistence, logging, and related glue), any `use` of `App\Domain\{Other}\…` is only allowed when it points at `App\Domain\{Other}\Contract\{Command|Query|Event|Entity|ValueObject|Enum|Exception}` and `{Other}` is not the adapter’s inferred home module (`InfrastructureCrossDomainImportsRule`, `InfrastructureCrossDomainGroupUseImportsRule`).
