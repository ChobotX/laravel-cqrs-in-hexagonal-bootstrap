...
The `SendNotificationHandler` resolves channels per recipient based on their preferences (defaults: info/success→in_app only, warning/error→in_app+email). In-app notifications are persisted directly by the handler. External channels (email, future push) use the `NotificationChannelSender` contract, resolved via `NotificationChannelSenderRegistry`. The handler loads each recipient’s user via `GetUserByIdQuery` on the query bus and silently skips missing recipients. The infrastructure email sender dispatches `SendTemplatedEmailCommand` via the command bus for the `notification` template type; missing templates raise `EmailTemplateNotFoundException` from the email-template module (a configuration failure, not a silent skip).

## Recipient Resolution

For team/subteam targeting, the `RecipientResolver` contract resolves team hierarchies to user ID lists. The default implementation is `DefaultRecipientResolver` in `Domain\Notification\Service`, which dispatches `ListTeamMemberUserIdsQuery` / `ListTeamSubtreeMemberUserIdsQuery` through the query bus; the Team module owns the recursive CTE and direct membership lookups. The domain handler receives pre-resolved `list<string>` of recipient IDs.
