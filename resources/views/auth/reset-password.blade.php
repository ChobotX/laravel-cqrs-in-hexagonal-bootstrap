...
@section('title', __('messages.auth.reset_password'))

@section('content')
    <h2 class="text-center text-xl font-semibold text-gray-900">{{ __('messages.auth.reset_password_title') }}</h2>
    <p class="mb-6 mt-1 text-center text-base text-gray-500 sm:text-sm">{{ __('messages.auth.reset_password_subtitle') }}</p>
    ...
    <form class="space-y-5"
        ...
        <x-primary-button skip-permission
                          variant="login"
                          :label="__('messages.auth.reset_password')" />
    </form>
@endsection
