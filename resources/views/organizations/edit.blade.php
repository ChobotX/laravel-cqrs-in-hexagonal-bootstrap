...
@section('title', __('messages.organizations.edit'))

@section('content')
    <div class="max-w-4xl">
        <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5">
            <div class="border-b border-gray-200 px-6 py-4">
                <p class="text-base text-gray-500 sm:text-sm">{{ __('messages.organizations.edit_subtitle') }}</p>
            </div>
            ...
            <form class="space-y-5 p-6"
                ...
                <div>
                    <label class="mb-1.5 block text-base font-medium text-gray-700 sm:text-sm"
                           for="name">{{ __('messages.organizations.name') }}</label>
                ...
                <div>
                    <label class="mb-1.5 block text-base font-medium text-gray-700 sm:text-sm"
                           for="slug">{{ __('messages.organizations.slug') }}</label>
                    <input class="block w-full max-w-md rounded-lg border border-gray-300 px-3.5 py-2.5 text-base shadow-sm focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600 sm:text-sm"
                           id="slug"
                           name="slug"
                           type="text"
                    ...
                    <p class="mt-1 text-xs text-gray-400">{{ __('messages.organizations.slug_hint') }}</p>
                    @error('slug')
                        <p class="mt-1 text-base text-red-600 sm:text-sm"
                ...
                <div>
                    <label class="mb-1.5 block text-base font-medium text-gray-700 sm:text-sm"
                           for="description">{{ __('messages.organizations.description') }}</label>
                    <textarea class="block w-full max-w-md rounded-lg border border-gray-300 px-3.5 py-2.5 text-base shadow-sm focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600 sm:text-sm"
                              ...
                              name="description"
                              rows="3"
                              @error('description') aria-describedby="description-error" aria-invalid="true" @enderror>{{ old('description', $organization->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-base text-red-600 sm:text-sm"
                ...
                <div class="flex items-center gap-3 pt-2">
                    <x-primary-button skip-permission
                                      :label="__('messages.organizations.update_action')" />
                    <x-primary-button skip-permission
                                      variant="secondary"
                                      :href="route('organizations.index')"
                                      :label="__('messages.organizations.cancel')" />
                </div>
            </form>
        </div>
    </div>
