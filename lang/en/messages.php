<?php

declare(strict_types=1);

return [
    'breadcrumb' => [
        'navigation' => 'Breadcrumb',
    ],

    'nav' => [
        'group_users_access' => 'Users & access',
        'group_email' => 'Email',
        'group_platform' => 'Feature flags & settings',
        'management' => 'Management',
        'users' => 'Users',
        'roles' => 'Roles',
        'teams' => 'Teams',
        'registry' => 'Registry',
        'notifications' => 'Notifications',
        'settings' => 'Settings',
        'password_rotation' => 'Password rotation',
        'dashboard' => 'Dashboard',
        'system' => 'System',
        'feature_flags' => 'Feature Flags',
        'sso' => 'Single Sign-On',
        'audit_log' => 'Audit Log',
        'email_templates' => 'Email Templates',
        'email_logs' => 'Sent Emails',
        'logout' => 'Log out',
    ],

    'dashboard' => [
        'welcome' => 'Welcome back, :name!',
        'users' => 'Users',
        'roles' => 'Roles',
        'teams' => 'Teams',
    ],

    'profile' => [
        'title' => 'My Profile',
        'subtitle' => 'Update your account information',
        'info_title' => 'Profile Information',
        'password_title' => 'Change Password',
        'password_subtitle' => 'Enter your current password to set a new one, or leave all fields blank',
        'two_factor_title' => 'Two-factor authentication',
        'two_factor_subtitle' => 'Set up authenticator app or email verification for your account.',
        'manage_two_factor' => 'Manage two-factor authentication',
        'current_password' => 'Current Password',
        'updated' => 'Profile updated.',
    ],

    'auth' => [
        'login' => 'Login',
        'sign_in_title' => 'Sign in to your account',
        'sign_in_subtitle' => 'Enter your credentials to continue',
        'email' => 'Email',
        'password' => 'Password',
        'sign_in' => 'Sign in',
        'invalid_credentials' => 'Invalid credentials.',
        'invalid_two_factor_code' => 'Invalid two-factor code.',
        'two_factor_title' => 'Two-factor verification',
        'two_factor_subtitle' => 'Enter the one-time code to finish sign in.',
        'send_email_code' => 'Send email code',
        'two_factor_method' => 'Verification method',
        'two_factor_code' => 'Verification code',
        'verify_two_factor' => 'Verify',
        'two_factor_code_sent' => 'A verification code has been sent to your email.',
        'two_factor_email_subject' => 'Your two-factor code',
        'two_factor_email_code' => 'Your verification code is :code.',
        'session_expired' => 'Your session has expired. Please log in again.',
        'accept_invite' => 'Accept Invite',
        'accept_invite_title' => 'Set your password',
        'accept_invite_subtitle' => 'Welcome :name, please set your password to activate your account.',
        'confirm_password' => 'Confirm password',
        'set_password' => 'Set password',
        'invite_accepted' => 'Your account has been activated.',
        'forgot_password' => 'Forgot password?',
        'forgot_password_title' => 'Reset your password',
        'forgot_password_subtitle' => 'Enter your email address and we will send you a reset link.',
        'send_reset_link' => 'Send reset link',
        'reset_link_sent' => 'If an account exists with that email, we have sent a password reset link.',
        'reset_password' => 'Reset Password',
        'reset_password_title' => 'Set a new password',
        'reset_password_subtitle' => 'Enter your new password below.',
        'password_reset_success' => 'Your password has been reset.',
    ],

    'email' => [
        'invite_subject' => 'You have been invited',
        'invite_body' => "You have been invited to join the platform.\n\nClick the link below to set your password and activate your account:\n\n:link\n\nThis link expires in 72 hours.",
        'password_reset_subject' => 'Password Reset Request',
        'password_reset_body' => "You have requested to reset your password.\n\nClick the link below to set a new password:\n\n:link\n\nThis link expires in 60 minutes.\n\nIf you did not request this, you can safely ignore this email.",
    ],

    'users' => [
        'title' => 'Users',
        'count' => '{1} user|[2,*] users',
        'create' => 'Create User',
        'edit' => 'Edit User',
        'user' => 'User',
        'email' => 'Email',
        'actions' => 'Actions',
        'edit_action' => 'Edit',
        'delete_action' => 'Delete',
        'name' => 'Name',
        'password' => 'Password',
        'password_keep' => '(leave blank to keep current)',
        'confirm_password' => 'Confirm Password',
        'create_action' => 'Create',
        'update_action' => 'Update',
        'cancel' => 'Cancel',
        'create_subtitle' => 'Add a new user to the system',
        'info_title' => 'User Information',
        'edit_subtitle' => 'Update user information',
        'access_title' => 'Access Management',
        'access_subtitle' => 'Manage roles, team memberships and labels',
        'created' => 'User created.',
        'updated' => 'User updated.',
        'deleted' => 'User deleted.',
        'invite_resent' => 'Invite has been resent.',
        'resend_invite' => 'Resend Invite',
        'pending_activation' => 'This user has not yet activated their account.',
        'no_role' => 'No role',
        'delete_confirm_title' => 'Delete User',
        'delete_confirm_message' => 'Are you sure you want to delete :name? This action cannot be undone.',
        'avatar' => 'Avatar',
        'avatar_remove' => 'Remove avatar',
        'empty' => 'No users found.',
        'roles' => 'Roles',
        'roles_subtitle' => 'Assign roles to this user',
        'roles_search' => 'Search roles...',
        'permissions_action' => 'Permissions',
        'teams' => 'Teams',
        'teams_subtitle' => 'Manage team memberships',
        'teams_search' => 'Search teams...',
        'in_team' => 'in',
        'two_factor_reset_section_title' => 'Second-factor recovery',
        'two_factor_reset_section_subtitle' => 'Use when the user lost their authenticator app and backup codes. They can sign in again (if your policy allows) and must set up 2FA from scratch.',
        'two_factor_reset_action' => 'Reset second factor for this user',
        'two_factor_reset_confirm_title' => 'Reset second factor?',
        'two_factor_reset_confirm_message' => 'This removes email OTP and authenticator app setup for :name. It cannot be undone.',
        'two_factor_reset' => 'Second factor was reset for this user.',
    ],

    'dialog' => [
        'cancel' => 'Cancel',
        'confirm' => 'Confirm',
        'ok' => 'OK',
    ],

    'a11y' => [
        'skip_to_content' => 'Skip to content',
        'language_switcher' => 'Language',
        'main_navigation' => 'Main navigation',
        'open_menu' => 'Open menu',
        'close_menu' => 'Close menu',
    ],

    'roles' => [
        'title' => 'Roles',
        'count' => '{1} role|[2,*] roles',
        'create' => 'Create Role',
        'edit' => 'Edit Role',
        'name' => 'Name',
        'description' => 'Description',
        'permissions' => 'Permissions',
        'actions' => 'Actions',
        'view_action' => 'View',
        'edit_action' => 'Edit',
        'delete_action' => 'Delete',
        'create_action' => 'Create',
        'update_action' => 'Update',
        'back' => 'Back',
        'cancel' => 'Cancel',
        'create_subtitle' => 'Define a new role with permissions',
        'edit_subtitle' => 'Update role permissions',
        'created' => 'Role created.',
        'updated' => 'Role updated.',
        'deleted' => 'Role deleted.',
        'delete_confirm_title' => 'Delete Role',
        'delete_confirm_message' => 'Are you sure you want to delete :name? This action cannot be undone.',
        'empty' => 'No roles found.',
        'system_badge' => 'System',
        'scope' => 'Scope',
        'module' => 'Module / Feature',
        'read' => 'Read',
        'create_perm' => 'Create',
        'update_perm' => 'Update',
        'delete_perm' => 'Delete',
    ],

    'permissions' => [
        'title' => 'User Permissions',
        'subtitle' => 'Manage roles and permission overrides for :name',
        'assigned_roles' => 'Assigned Roles',
        'add_role' => 'Add Role',
        'remove_role' => 'Remove Role',
        'effective_permissions' => 'Effective Permissions',
        'overrides' => 'Add Override',
        'active_overrides' => 'Active Overrides',
        'add_override' => 'Add Override',
        'remove_override' => 'Remove Override',
        'source' => 'Source',
        'scope' => 'Scope',
        'granted' => 'Granted',
        'denied' => 'Denied',
        'not_granted' => 'Not granted',
        'deny_tooltip' => 'Denied by user override — overrides role grant from :source',
        'updated' => 'Permissions updated.',
    ],

    'impersonation' => [
        'start' => 'Impersonate',
        'stop' => 'Stop Impersonation',
        'banner' => 'You are viewing as :name — ',
        'started' => 'Impersonation started.',
        'stopped' => 'Impersonation stopped.',
    ],

    'scopes' => [
        'all' => 'All',
        'team_tree' => 'Team Tree + Shared',
        'team' => 'Direct Team + Shared',
        'own' => 'Own + Shared',
    ],

    'errors' => [
        'go_back' => 'Go back',
    ],

    'teams' => [
        'title' => 'Teams',
        'count' => '{1} team|[2,*] teams',
        'create' => 'Create Team',
        'edit' => 'Edit Team',
        'name' => 'Name',
        'slug' => 'Slug',
        'description' => 'Description',
        'parent' => 'Parent Team',
        'no_parent' => 'None (root team)',
        'actions' => 'Actions',
        'view_action' => 'View',
        'edit_action' => 'Edit',
        'delete_action' => 'Delete',
        'create_action' => 'Create',
        'update_action' => 'Update',
        'back' => 'Back',
        'cancel' => 'Cancel',
        'create_subtitle' => 'Create a new team',
        'edit_subtitle' => 'Update team details',
        'created' => 'Team created.',
        'updated' => 'Team updated.',
        'deleted' => 'Team deleted.',
        'members_updated' => 'Team membership updated.',
        'delete_confirm_title' => 'Delete Team',
        'delete_confirm_message' => 'Are you sure you want to delete :name? Children will be reparented.',
        'empty' => 'No teams found.',
        'slug_hint' => 'Lowercase letters, numbers, and hyphens only (2-63 characters)',
        'members' => 'Members',
        'add_member' => 'Add Member',
        'remove_member' => 'Remove Member',
        'no_members' => 'No members yet.',
        'search_member' => 'Search users...',
        'no_results' => 'No results found.',
        'view_list' => 'List view',
        'view_tree' => 'Tree view',
    ],

    'autocomplete' => [
        'search_failed' => 'Search failed. Please try again.',
    ],

    'labels' => [
        'title' => 'Labels',
        'subtitle' => 'Manage labels for this record',
        'search' => 'Search labels...',
        'no_results' => 'No labels found.',
        'create' => 'Create "%s"',
        'created' => 'Label created.',
        'create_failed' => 'Failed to create label.',
        'search_failed' => 'Failed to load labels.',
    ],

    'sharing' => [
        'title' => 'Sharing',
        'subtitle' => 'Share this record with other users. Shared users can view and edit it.',
        'loading' => 'Loading shares...',
        'search_placeholder' => 'Search users to share with...',
        'no_shares' => 'This record is not shared with anyone.',
        'revoke' => 'Revoke',
        'revoke_confirm' => "Remove :name's access to this record?",
        'shared_success' => 'Record shared successfully.',
        'revoked_success' => 'Access revoked.',
        'share_failed' => 'Failed to share record.',
        'revoke_failed' => 'Failed to revoke access.',
        'load_failed' => 'Failed to load shares.',
        'search_failed' => 'Failed to search users.',
    ],

    'feature_flags' => [
        'title' => 'Feature Flags',
        'flag' => 'Flag',
        'type' => 'Type',
        'value' => 'Value',
        'status' => 'Status',
        'enabled' => 'Enabled',
        'actions' => 'Actions',
        'default_value' => 'Default Value',
        'edit' => 'Edit Feature Flag',
        'edit_action' => 'Edit',
        'save' => 'Save',
        'cancel' => 'Cancel',
        'reset_action' => 'Reset to Default',
        'reset_to_default' => 'Reset to Default',
        'reset_confirm_title' => 'Reset Feature Flag',
        'reset_confirm_message' => 'Are you sure you want to reset :key to its default value?',
        'updated' => 'Feature flag updated.',
        'reset' => 'Feature flag reset to default.',
        'override_active' => 'This flag has a tenant override. Default value: :default',
        'override_badge' => 'Override',
        'default_badge' => 'Default',
        'type_boolean' => 'Toggle',
        'type_select' => 'Select',
        'type_input' => 'Input',
        'pattern_hint' => 'Must match pattern: :pattern',
        'empty' => 'No feature flags configured.',
        'count' => '{1} flag|[2,*] flags',
        'group_filter_all' => 'All Groups',
        'group_filter_label' => 'Filter by group',
        'groups' => [
            'registry' => [
                'label' => 'Registry',
                'flags' => [
                    'schema-builder' => [
                        'label' => 'Schema Builder',
                        'description' => 'Enable the Schema/Entry system for dynamic data definitions',
                    ],
                ],
            ],
            'sso' => [
                'label' => 'Single Sign-On',
                'flags' => [
                    'enabled' => [
                        'label' => 'Enable SSO',
                        'description' => 'Show per-tenant SSO providers on the login page and expose the admin UI.',
                    ],
                ],
            ],
        ],
    ],

    'settings' => [
        'title' => 'Tenant Settings',
        'subtitle' => 'Manage your organization name and branding',
        'display_timezone' => 'Display timezone',
        'display_timezone_hint' => 'Dates and times in the app use this timezone. Leave empty to use each visitor’s browser timezone.',
        'display_timezone_browser' => 'Browser default (no fixed timezone)',
        'invalid_display_timezone' => 'Choose a valid IANA timezone or leave the field empty.',
        'name' => 'Organization Name',
        'logo' => 'Logo',
        'current_logo' => 'Current logo',
        'remove_logo' => 'Remove logo',
        'logo_hint' => 'PNG, JPG or SVG. Max 2 MB.',
        'update_action' => 'Save',
        'cancel' => 'Cancel',
        'updated' => 'Settings updated.',
        'password_rotation_title' => 'Password rotation',
        'password_rotation_intro' => 'Require users to change passwords periodically and block reuse of recent passwords.',
        'rotation_enabled' => 'Enable password expiry',
        'max_age_days' => 'Maximum password age (days)',
        'history_count' => 'Passwords remembered for reuse prevention',
        'password_rotation_warning' => 'Your password will expire soon. Please change it.',
        'password_rotation_expired' => 'Your password has expired. You must change it to continue.',
        'change_password_link' => 'Change password',
        'password_rotation_updated' => 'Password rotation settings saved.',
        'two_factor_title' => 'Two-factor authentication',
        'two_factor_intro' => 'Configure and enforce two-factor authentication methods.',
        'own_two_factor_intro' => 'Set up and manage second-factor sign-in for your account.',
        'two_factor_required_for_all' => 'Require two-factor for all users',
        'two_factor_email_method' => 'Allow Email OTP',
        'two_factor_totp_method' => 'Allow Authenticator App (TOTP)',
        'two_factor_updated' => 'Two-factor settings saved.',
        'confirm_totp_setup' => 'Confirm authenticator code',
        'cancel_totp_setup' => 'Cancel authenticator setup',
        'email_otp_section_title' => 'Email one-time password (OTP)',
        'email_otp_section_subtitle' => 'Use email-delivered one-time codes as your second factor.',
        'email_otp_switch_label' => 'Email one-time codes',
        'email_otp_disabled_by_tenant' => 'Your organization does not allow email OTP for two-factor authentication.',
        'totp_section_title' => 'Authenticator app (TOTP)',
        'totp_section_subtitle' => 'Use a time-based one-time password app as your second factor.',
        'totp_switch_label' => 'Authenticator app',
        'totp_disabled_by_tenant' => 'Your organization does not allow authenticator apps for two-factor authentication.',
        'totp_enabled_active_hint' => 'Authenticator app is enabled. Turn the switch off to remove it.',
        'totp_qr_hint' => 'Scan this QR code in Google Authenticator, 1Password, Authy, or another TOTP app.',
        'totp_qr_aria' => 'Authenticator setup QR code',
        'totp_secret_label' => 'Cannot scan the QR code? Enter this key manually',
        'totp_backup_codes_title' => 'Recovery codes',
        'totp_backup_codes_instruction' => 'Save these one-time recovery codes in a safe place. You need them if you lose access to your authenticator app.',
        'totp_backup_codes_download' => 'Download recovery codes (.txt)',
        'totp_backup_codes_download_required_hint' => 'Download your recovery codes before you confirm the authenticator code.',
        'totp_backup_codes_downloaded_ack' => 'Download recorded. You can now confirm your authenticator code.',
        'totp_backup_codes_download_required' => 'Download your recovery codes before confirming authenticator setup.',
        'totp_code_label' => 'Authenticator code',
        'mail_title' => 'Email provider',
        'mail_intro' => 'Override the platform default SMTP transport with your own provider so invites and password resets are sent from your address.',
        'mail_use_custom' => 'Use a custom email provider for this tenant',
        'mail_default_in_use' => 'Tenant emails are currently sent through the platform default transport.',
        'mail_provider' => 'Provider',
        'mail_provider_mailpit' => 'Mailpit (local development)',
        'mail_provider_mailjet' => 'Mailjet',
        'mail_provider_mailgun' => 'Mailgun',
        'mail_provider_custom' => 'Custom SMTP',
        'mail_host' => 'SMTP host',
        'mail_port' => 'SMTP port',
        'mail_encryption' => 'Encryption',
        'mail_encryption_none' => 'None',
        'mail_username' => 'SMTP username',
        'mail_password' => 'SMTP password / API secret',
        'mail_password_set_placeholder' => '•••••••• (kept)',
        'mail_password_hint' => 'Leave empty to keep the existing password.',
        'mail_from_address' => 'From address',
        'mail_from_name' => 'From name',
        'mail_updated' => 'Email provider settings saved.',
        'mail_test_action' => 'Send test email',
        'mail_test_hint' => 'Sends a test email to your address using the configuration above.',
        'mail_test_sent' => 'Test email sent. Check your inbox.',
        'mail_test_failed' => 'Test email failed: :error',
    ],

    'audit_log' => [
        'title' => 'Audit Log',
        'action' => 'Action',
        'user' => 'User',
        'entity' => 'Entity',
        'trace_id' => 'Trace ID',
        'occurred_at' => 'Time',
        'ip_address' => 'IP Address',
        'status' => 'Status',
        'payload' => 'Details',
        'success' => 'Success',
        'failure' => 'Failure',
        'no_entries' => 'No audit entries found.',
        'filter_entity_type' => 'Entity Type',
        'filter_user' => 'User ID',
        'filter_trace' => 'Trace ID',
        'filter_from' => 'From',
        'filter_to' => 'To',
        'filter' => 'Filter',
        'clear' => 'Clear',
        'trace_title' => 'Trace :traceId',
        'back' => 'Back to Audit Log',
        'changes' => 'Changes',
        'no_changes' => 'No changes',
        'redacted' => 'redacted',
        'changes_count' => ':count change|:count changes',
        'grouped_entries' => '{1} action in this request|[2,*] actions in this request',
    ],

    'exceptions' => [
        'user_not_found' => 'User with id [:id] not found.',
        'invalid_user_id' => 'Value [:value] is not a valid UUID.',
        'invalid_email' => 'Value [:value] is not a valid email address.',
        'invalid_user_data_empty_name' => 'User name must not be empty.',
        'email_already_exists' => 'A user with email [:email] already exists.',
        'role_not_found' => 'Role with id [:id] not found.',
        'role_already_exists' => 'A role with name [:name] already exists.',
        'invalid_permission_key' => 'Value [:value] is not a valid permission key.',
        'invalid_role_id' => 'Value [:value] is not a valid role UUID.',
        'invalid_role_name' => 'Role name must not be empty.',
        'duplicate_role_assignment' => 'This role is already assigned to the user.',
        'role_not_assigned' => 'This role is not assigned to the user.',
        'record_share_not_found' => 'The record share does not exist.',
        'permission_denied' => 'You do not have permission to perform this action.',
        'impersonation_not_allowed' => 'You are not allowed to impersonate users.',
        'invalid_team_id' => 'Value [:value] is not a valid team UUID.',
        'invalid_team_name' => 'Team name must not be empty.',
        'invalid_team_slug' => 'Value [:value] is not a valid team slug.',
        'team_not_found' => 'Team with id [:id] not found.',
        'team_slug_already_exists' => 'A team with slug [:slug] already exists.',
        'team_member_already_exists' => 'This user is already a member of this team.',
        'team_member_not_found' => 'This user is not a member of this team.',
        'team_cycle_detected' => 'This parent would create a circular hierarchy.',
        'invalid_label_id' => 'Value [:value] is not a valid label UUID.',
        'invalid_label_name' => 'Label name [:value] is not valid.',
        'invalid_label_namespace' => 'Label namespace [:value] is not valid.',
        'label_already_exists' => 'A label [:name] already exists.',
        'label_not_found' => 'Label not found.',
        'label_namespace_mismatch' => 'This label does not belong to this entity type.',
        'invalid_notification_id' => 'Value [:value] is not a valid notification UUID.',
        'notification_not_found' => 'Notification with id [:id] not found.',
        'notification_email_template_not_found' => 'The notification email template is not configured for locale [:locale] or fallback [:fallbackLocale].',
        'notification_ownership' => 'You do not have access to this notification.',
        'invalid_notification_type' => 'Value [:value] is not a valid notification type.',
        'invalid_notification_link' => 'Value [:value] is not a valid notification link.',
        'concurrent_modification' => 'This record was modified by another user. Please reload and try again.',
        'invalid_file_id' => 'Value [:value] is not a valid file UUID.',
        'invalid_file_name' => 'Value [:value] is not a valid file name.',
        'invalid_file_namespace' => 'Value [:value] is not a valid file namespace.',
        'invalid_mime_type' => 'Value [:value] is not a valid MIME type.',
        'invalid_storage_path' => 'Value [:value] is not a valid storage path.',
        'invalid_file_version' => 'Value [:value] is not a valid file version.',
        'file_not_found' => 'File with id [:id] not found.',
        'file_storage_error' => 'File storage operation failed.',
        'image_processing_error' => 'Image processing failed.',
        'tenant_not_found' => 'Tenant with id [:id] not found.',
        'tenant_display_name_not_configured' => 'Organization display name is not configured for this tenant.',
        'invalid_tenant_name' => 'Tenant name must not be empty.',
        'invalid_tenant_display_timezone' => 'The display timezone is not a valid IANA identifier.',
        'tenant_logo_storage_failed' => 'Failed to store tenant logo.',
        'tenant_inactive' => 'Tenant [:identifier] is inactive.',
        'invalid_password_reset_token' => 'This password reset link is invalid or has expired.',
        'user_already_activated' => 'This account has already been activated.',
        'feature_flag_not_found' => 'Feature flag [:key] not found.',
        'invalid_flag_key' => 'Invalid feature flag key format: [:key].',
        'invalid_flag_value' => 'Invalid value [:value] for feature flag [:key]: :reason',
        'invalid_id' => 'Value [:value] is not a valid UUID.',
        'grid_preset_not_found' => 'Grid preset not found.',
        'grid_preset_ownership' => 'You do not have access to this grid preset.',
        'grid_preset_scope_permission' => 'You do not have permission to share presets with this scope.',
        'invalid_audit_log_id' => 'Value [:value] is not a valid UUID.',
        'email_template_not_found' => 'Email template [:type] for locale [:locale] not found.',
        'invalid_email_template_type' => 'Value [:value] is not a valid email template type.',
        'invalid_email_template_locale' => 'Value [:value] is not a valid locale.',
        'invalid_password_rotation_max_age' => 'When expiry is enabled, password age must be between 1 and 3660 days.',
        'invalid_password_rotation_history' => 'History length must be between 1 and 24 previous passwords.',
        'invalid_mail_transport_host' => 'SMTP host must not be empty.',
        'invalid_mail_transport_port' => 'SMTP port must be between 1 and 65535.',
        'invalid_mail_transport_from_address' => 'From address must be a valid email.',
        'invalid_mail_transport_from_name' => 'From name must not be empty.',
        'invalid_mail_transport_encryption' => 'Encryption must be empty, "tls", or "ssl".',
        'invalid_two_factor_policy_requires_method' => 'At least one two-factor method must stay enabled when enforcement is required.',
        'password_previously_used' => 'Choose a password you have not used recently.',
        'invalid_sso_configuration_id' => 'Value [:value] is not a valid SSO configuration UUID.',
        'invalid_sso_slug' => 'Value [:value] is not a valid SSO slug.',
        'sso_configuration_not_found' => 'SSO configuration with id [:id] not found.',
        'sso_configuration_conflict' => 'An SSO configuration with provider [:providerType] and slug [:slug] already exists.',
        'sso_login_rejected' => 'SSO login could not be completed: :reason.',
        'sso_enforcement_violation' => 'Password login is disabled for this tenant; sign in via SSO.',
        'sso_identity_not_found' => 'SSO identity with id [:id] not found.',
    ],

    'sso' => [
        'title' => 'Single Sign-On',
        'create' => 'Add SSO provider',
        'create_action' => 'Create',
        'update_action' => 'Update',
        'edit' => 'Edit SSO provider',
        'edit_action' => 'Edit',
        'delete_action' => 'Delete',
        'test_action' => 'Test',
        'cancel' => 'Cancel',
        'empty' => 'No SSO providers configured.',
        'provider_type' => 'Provider type',
        'slug' => 'Slug',
        'display_name' => 'Display name',
        'jit_mode' => 'Provisioning mode',
        'allowed_email_domains' => 'Allowed email domains (comma-separated)',
        'enabled' => 'Enabled',
        'enforce' => 'Enforce SSO (disable password login for non-admins)',
        'created' => 'SSO provider created.',
        'updated' => 'SSO provider updated.',
        'deleted' => 'SSO provider deleted.',
        'or_continue_with' => 'Or continue with',
        'redirecting' => 'Redirecting to your identity provider…',
        'continue_to_idp' => 'Continue to identity provider',
    ],

    'email_templates' => [
        'title' => 'Email Templates',
        'subtitle' => 'Manage the content and appearance of system emails',
        'count' => '{1} template|[2,*] templates',
        'name' => 'Name',
        'description' => 'Description',
        'last_modified' => 'Last Modified',
        'actions' => 'Actions',
        'edit_action' => 'Edit',
        'edit' => 'Edit Email Template',
        'edit_subtitle' => 'Customize the email template content and layout',
        'type_label' => 'Template Type',
        'locale_label' => 'Locale',
        'subject' => 'Subject',
        'body' => 'Body',
        'available_variables' => 'Available Variables',
        'sensitive_badge' => 'Sensitive',
        'preview' => 'Preview',
        'save' => 'Save',
        'cancel' => 'Cancel',
        'reset_to_default' => 'Reset to Default',
        'reset_confirm_title' => 'Reset Email Template',
        'reset_confirm_message' => 'Are you sure you want to reset this template to its default content?',
        'updated' => 'Email template updated.',
        'reset' => 'Email template reset to default.',
        'empty' => 'No email templates found.',
        'source_view' => 'HTML',
        'visual_view' => 'Visual',
        'unlink' => 'Unlink',
        'email_footer' => 'This email was sent by :tenant.',
        'switch_locale' => 'Switch to :locale',
        'types' => [
            'user_invite' => [
                'name' => 'User Invitation',
                'description' => 'Sent when a new user is invited to the platform',
            ],
            'password_reset' => [
                'name' => 'Password Reset',
                'description' => 'Sent when a user requests a password reset',
            ],
            'two_factor_challenge' => [
                'name' => 'Two-Factor Email Challenge',
                'description' => 'Sent when a user requests an email one-time code during two-factor challenge',
            ],
            'notification' => [
                'name' => 'Notification Email',
                'description' => 'Wraps in-app notifications sent via email channel',
            ],
        ],
    ],

    'email_logs' => [
        'title' => 'Sent Emails',
        'subtitle' => 'Review sent email history',
        'count' => '{1} email|[2,*] emails',
        'recipient' => 'Recipient',
        'template_type' => 'Template',
        'subject' => 'Subject',
        'sent_at' => 'Sent At',
        'expand' => 'Expand',
        'toggle_details' => 'Toggle details',
        'details' => 'Details',
        'body_preview' => 'Email Body (sensitive data masked)',
        'variables_used' => 'Variables Used',
        'masked_body' => 'Email Body (sensitive data masked)',
        'variable_keys' => 'Variables Used',
        'trace_id' => 'Trace ID',
        'empty' => 'No emails have been sent yet.',
    ],

    'pagination' => [
        'navigation' => 'Pagination',
        'showing' => 'Showing :from to :to of :total results',
        'previous' => 'Previous',
        'next' => 'Next',
        'page' => 'Page :page',
    ],

    'grid' => [
        'search_placeholder' => 'Search...',
        'search_label' => 'Search',
        'filter_all' => 'All',
        'clear_filters' => 'Clear all',
        'remove_filter' => 'Remove filter: :label',
        'search_filter' => 'Search: :term',
        'no_results' => 'No results found for your search.',
        'actions' => 'Actions',
        'presets' => [
            'save' => 'Save view',
            'save_as' => 'Save as new view',
            'name_label' => 'View name',
            'name_placeholder' => 'My view',
            'rename' => 'Rename',
            'set_default' => 'Set as default',
            'delete' => 'Delete view',
            'delete_confirm_title' => 'Delete View',
            'delete_confirm_message' => 'Are you sure you want to delete the view ":name"?',
            'delete_shared_warning' => 'This preset is shared with others. Deleting it will remove it for everyone.',
            'all' => 'All',
            'scope_personal' => 'Only me',
            'scope_team' => 'My team',
            'scope_global' => 'Everyone',
            'share_label' => 'Visible to',
            'scope_badge_team' => 'Team',
            'scope_badge_global' => 'Global',
        ],
    ],

    'dialog' => [
        'cancel' => 'Cancel',
        'confirm' => 'Confirm',
    ],

    'registry' => [
        'title' => 'Registry',
        'definitions' => [
            'title' => 'Definitions',
            'count' => '{1} definition|[2,*] definitions',
            'create' => 'Create Definition',
            'edit' => 'Edit Definition',
            'create_subtitle' => 'Define a new data type with namespace and slug',
            'edit_subtitle' => 'Update definition name and manage schema versions',
            'create_action' => 'Create',
            'name' => 'Name',
            'namespace' => 'Namespace',
            'slug' => 'Slug',
            'slug_hint' => 'Lowercase letters, numbers, and underscores only',
            'active_version' => 'Active Version',
            'actions' => 'Actions',
            'view_action' => 'View',
            'edit_action' => 'Edit',
            'delete_action' => 'Delete',
            'cancel' => 'Cancel',
            'back' => 'Back',
            'empty' => 'No definitions found.',
            'created' => 'Definition created.',
            'updated' => 'Definition updated.',
            'deleted' => 'Definition deleted.',
            'delete_confirm_title' => 'Delete Definition',
            'delete_confirm_message' => 'Are you sure you want to delete :name? This action cannot be undone.',
            'update_name' => 'Update Name',
            'entries_link' => 'View Entries',
        ],
        'versions' => [
            'title' => 'Schema Versions',
            'version' => 'Version',
            'status' => 'Status',
            'fields' => 'Fields',
            'actions' => 'Actions',
            'create_action' => 'New Version',
            'activate_action' => 'Activate',
            'deprecate_action' => 'Deprecate',
            'deprecate_confirm_title' => 'Deprecate this version?',
            'deprecate_confirm_message' => 'This version will be marked as deprecated and will no longer be available for new entries.',
            'no_versions' => 'No versions yet.',
            'created' => 'Version created.',
            'activated' => 'Version activated.',
            'deprecated' => 'Version deprecated.',
            'status_draft' => 'Draft',
            'status_active' => 'Active',
            'status_deprecated' => 'Deprecated',
            'type_string' => 'String',
            'type_integer' => 'Integer',
            'type_number' => 'Number',
            'type_boolean' => 'Boolean',
            'type_date' => 'Date',
            'type_email' => 'Email',
            'type_reference' => 'Reference',
            'type_file' => 'File',
            'type_repeater' => 'Repeater',
            'type_object' => 'Object',
            'select_definition' => 'Select definition',
            'schema_builder' => 'Schema Builder',
            'add_field' => 'Add Field',
            'remove_field' => 'Remove Field',
            'field_name' => 'Field Name',
            'field_label' => 'Field Label',
            'field_type' => 'Field Type',
            'field_required' => 'Required',
            'multiline' => 'Multiline',
            'display_label_placeholder' => 'Display Label',
            'field_name_placeholder' => 'field_name',
            'file_namespace' => 'File Namespace',
            'file_mime_types' => 'MIME Types',
            'file_max_size' => 'Max Size (B)',
            'min_items' => 'Min Items',
            'max_items' => 'Max Items',
            'sub_fields' => 'Sub-fields',
            'properties' => 'Properties',
            'configure_field' => 'Configure',
            'close' => 'Close',
            'section_type_settings' => 'Type Settings',
            'section_validation' => 'Validation',
            'section_presentation' => 'Presentation',
            'placeholder' => 'Placeholder',
            'help_text' => 'Help Text',
            'default_value' => 'Default Value',
            'default_checked' => 'Checked by default',
            'min_length' => 'Min Length',
            'max_length' => 'Max Length',
            'pattern' => 'Pattern (regex)',
            'min_value' => 'Minimum',
            'max_value' => 'Maximum',
            'step' => 'Step',
            'min_date' => 'Earliest Date',
            'max_date' => 'Latest Date',
        ],
        'entries' => [
            'title' => 'Entries',
            'count' => '{1} entry|[2,*] entries',
            'create' => 'Create Entry',
            'create_subtitle' => 'Add a new entry for this definition',
            'edit' => 'Edit Entry',
            'edit_subtitle' => 'Update entry data',
            'create_action' => 'Create',
            'update_action' => 'Update',
            'delete_action' => 'Delete',
            'cancel' => 'Cancel',
            'title_field' => 'Title',
            'version_field' => 'Version',
            'actions' => 'Actions',
            'empty' => 'No entries found.',
            'add_row' => 'Add row',
            'created' => 'Entry created.',
            'updated' => 'Entry updated.',
            'deleted' => 'Entry deleted.',
            'delete_confirm_title' => 'Delete Entry',
            'delete_confirm_message' => 'Are you sure you want to delete :name? This action cannot be undone.',
        ],
    ],

    'notifications' => [
        'title' => 'Notifications',
        'subtitle' => 'View and manage your notifications',
        'mark_all_read' => 'Mark all as read',
        'view_all' => 'View all notifications',
        'empty' => 'No notifications yet.',
        'loading' => 'Loading...',
        'delete' => 'Delete',
        'filter_all' => 'All',
        'filter_unread' => 'Unread',
        'filter_read' => 'Read',
        'prev' => 'Previous',
        'next' => 'Next',
        'just_now' => 'Just now',
        'minutes_ago' => ':count min ago',
        'hours_ago' => ':count hr ago',
        'days_ago' => ':count d ago',
        'preferences_title' => 'Notification Preferences',
        'preferences_subtitle' => 'Choose how you receive notifications for each priority level',
        'preferences_level' => 'Level',
        'preferences_in_app' => 'In-App',
        'preferences_email' => 'Email',
        'mark_read' => 'Mark as read',
        'show_more' => 'Show more',
        'show_less' => 'Show less',
        'error_loading' => 'Failed to load notifications.',
        'error_action' => 'Notification action failed.',
        'levels' => [
            'info' => 'Info',
            'success' => 'Success',
            'warning' => 'Warning',
            'error' => 'Error',
        ],
    ],
];
