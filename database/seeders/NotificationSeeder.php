...
final class NotificationSeeder extends Seeder
{
    ...
    private function seedAdminExampleNotifications(): void
    {
        ...
        $notifications = [
            ...
            ['type' => 'system.error', 'title' => 'Background job failed', 'body' => 'The nightly report generation job failed after 3 retries. The job processes aggregated usage metrics for all active tenants and generates PDF summaries. Last successful run was 48 hours ago. Manual intervention may be required to clear the failed job from the queue and re-trigger the report generation.', 'level' => 'error', 'link' => null, 'is_read' => false, 'read_at' => null, 'created_at' => $now->copy()->subHours(3)],
