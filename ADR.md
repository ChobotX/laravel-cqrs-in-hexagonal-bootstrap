...
**Why:** A parallel "Shared" scope would double the branching in every repository. Folding shares into `Own`/`Team` scope via a UNION keeps the query model uniform and handlers unaware of sharing — the trait absorbs all the work.
**Enforced by:** Convention + `ShareableResourceRegistry` binding in `AuthorizationServiceProvider::register()`. See [app/Domain/Authorization/README.md](app/Domain/Authorization/README.md#record-sharing).
...
### Per-tenant SSO behind a single authenticator port

SSO is a tenant module (`App\Domain\Sso`). Each tenant may register multiple `SsoConfiguration` rows (OIDC / OAuth2 social / SAML 2.0), opt into SSO enforcement against password login, and choose how unknown subjects get provisioned (`invited_only`, `auto_create`, `linked_only`). The login flow is unified behind the `SsoAuthenticator` port with three protocol-specific adapters in Infrastructure (`SocialiteOidcAuthenticator`, `SocialiteSocialAuthenticator`, `Saml2Authenticator`) chosen at runtime by `SsoAuthenticatorRegistry`. Secrets live encrypted in the tenant `sso_configurations.config` column; there is no landlord storage of IdP credentials. JIT provisioning dispatches existing User commands (`CreateUserCommand`, `MarkUserActivatedCommand`) through the bus rather than writing to the user repository directly.
**Why:** A single port keeps presentation controllers protocol-agnostic — they only know how to redirect, accept callbacks, and dispatch `LoginViaSsoCommand`. Per-tenant configuration plus encrypted secrets keeps tenant isolation intact even for federated identity. Re-using existing User commands for provisioning preserves the audit/event trail.
**Enforced by:** `Domain/Sso` module rules (every command + query carries `RequiresPermission` or `SkipPermissionCheck`, every command handler collects an event), the `sso.enabled` feature flag gate, and `SsoAuthenticatorRegistry`'s exhaustive `match` on `ProviderType`.
